...
def robot_epoch(robot, gamma=0.7, theta=0.1, certainty=0.8):
    ...
    for i in range(rows):
        for j in range(cols):
            ...
            if inputgrid[j,i] < -2:  # if the robot is on the tile
                r[i,j] = 0  # we consider the tile clean
            elif inputgrid[j,i] == 3:  # if the tile is a death tile
                r[i,j] = -1  # we consider it as a wall
            else:
                r[i,j] = inputgrid[j,i]

    # 1 up, 2 right, 3 down, 4 left
    policy = np.random.randint(1, 5, size=(rows, cols))
    v_values = np.zeros((rows,cols))
        ...
        while delta >= theta:
            ...
            for i in range(rows):
                for j in range(cols):
                    ...
                    choice = policy[i,j]
                    if choice == 1:
                        v_values[i,j] = certainty * (v_values[i, j-1] + gamma**iteration + r[i, j]) + (1-certainty)/3 *\
                        ...
                        v_values[i,j] = certainty * (v_values[i+1, j] + gamma**iteration + r[i, j]) +\
                                        (1-certainty)/3 * \
                                        (r[i, j] * 3 + gamma**iteration * (v_values[i, j+1] + v_values[i-1, j] + v_values[i, j-1]))

                    elif choice == 3:
                        v_values[i,j] = certainty * (v_values[i, j+1] + gamma**iteration + r[i, j]) +\
                                        (1-certainty)/3 * \
                                        (r[i, j] * 3 + gamma**iteration * (v_values[i, j-1] + v_values[i-1, j] + v_values[i+1, j]))
    ...
    direction = ['n', 'e', 's', 'w'][policy[j_position, i_position] - 1]
    print(direction)
